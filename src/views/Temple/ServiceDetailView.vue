      ...
      <div class="form-card">
        <div class="card-header">
          <div style="display:flex;align-items:center;gap:12px">
            <h2 class="form-title" style="margin:0">服務資訊</h2>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <template v-if="isEditing">
              <input v-model="form.nameZhTw" class="form-input" placeholder="請輸入服務名稱" />
            </template>
          ...
          <div class="form-group">
            ...
            <div v-else class="field-value">{{ getLabelById(serviceItems, form.itemId) }}</div>
          </div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <div v-else class="field-value">{{ getLabelById(serviceCategories, form.categoryId) }}</div>
          </div>
          <div class="form-group">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <div v-else class="field-value">{{ form.isPermanent ? '常駐服務' : (form.publishAt || '-') }}</div>
          </div>
          <div class="form-group">
            ...
            <div v-else class="field-value">{{ form.isPermanent ? '常駐服務' : (form.unpublishAt || '-') }}</div>
          </div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group full-width">
            ...
            <div v-else class="field-value">{{ form.isPermanent ? '常駐服務' : '非常駐服務' }}</div>
          </div>
        </div>
        ...
        <div class="specs-list">
          <div v-for="(spec, idx) in form.specs" :key="spec.id" class="spec-card">
            ...
            <div class="spec-row-2col">
              <div class="spec-block">
                <label class="spec-label">服務價格（元）</label>
              ...
              <div class="spec-block">
                <label class="spec-label">報名數量</label>
                <template v-if="isEditing">
                ...
                <div v-else class="field-value">{{ spec.unlimitedQty ? '無限制' : spec.stock }}</div>
              </div>
            </div>
        ...
        <div class="form-row" style="margin-top: 24px;">
          <div class="form-group full-width">
            ...
            <textarea v-if="isEditing" v-model="form.depictionZhTw" class="form-textarea" placeholder="請輸入服務說明" rows="5"></textarea>
...
const form = reactive({
  status: '',
  itemId: '', nameZhTw: '', categoryId: '', eventIds: [],
  ritualDocumentId: null, certificateId: null, isInvoiceSupported: false,
  tags: [], publishAt: '', unpublishAt: '', isPermanent: false,
  depictionZhTw: '', specs: [],
})
...
const addSpec = () => {
  form.specs.push({
    id: ++specIdCounter, nameZhTw: '', price: '', stock: '',
    unlimitedQty: false, isCompanyPurchasable: false,
    shippingMode: 'FREE', specialShippingFee: 0, images: [],
  })
}
const removeSpec    = (idx) => form.specs.splice(idx, 1)
const onUnlimitedQtyChange = (spec) => { spec.stock = spec.unlimitedQty ? -1 : '' }
...
const fillForm = (data) => {
  ...
  // 快取活動名稱，供 view mode 顯示用（API 回傳 name 而非 nameZhTw）
  const namesMap = {}
  ...
  form.specs = (data.skus || []).map(sku => ({
    ...
    shippingMode: sku.shippingMode || 'FREE', specialShippingFee: sku.specialShippingFee ?? 0,
...
const handleSubmit = async () => {
  isSaving.value = true
  try {
    await templeStore.updateService(templeId.value, serviceId.value, buildPayload())
    alert('服務更新成功')
    isEditing.value = false
  } catch (err) {
    console.error('服務更新失敗:', err)
...
const openTagModal = () => {
  tagSearch.value = ''; isAddingTag.value = false
  newTagName.value = ''; editingTagId.value = null
...
const goBack = () => router.push({ name: 'app.temple.activity-management', params: { templeId: templeId.value } })
...
<style scoped>
.service-form-page { padding: 24px 32px 40px; min-height: 100%; background: #F3F4F6; }
...
.loading-state { text-align: center; padding: 80px; color: #aaa; font-size: 14px; }
...
.btn-edit {
  ...
  padding: 8px 20px; background: #fff; border: 1.5px solid #E8572A;
  border-radius: 50px; color: #E8572A; font-size: 13px; font-weight: 500;
...
.btn-cancel-top {
  display: inline-flex; align-items: center; gap: 6px;
  padding: 8px 20px; background: #fff; border: 1.5px solid #d1d5db;
  border-radius: 50px; color: #555; font-size: 13px; font-weight: 500;
  cursor: pointer; transition: background 0.15s;
}
...
/* 純文字顯示 */
.field-value { font-size: 14px; color: #1a1a1a; min-height: 42px; display: flex; align-items: center; }
.field-value.pre-wrap { white-space: pre-wrap; align-items: flex-start; min-height: unset; line-height: 1.7; }
...
.form-select {
  ...
  appearance: none; cursor: pointer; outline: none; transition: border-color 0.2s; box-sizing: border-box;
}
...
.spec-card { position: relative; background: #fff; border: 1px solid #e8e8e8; border-radius: 14px; padding: 28px 32px; margin-bottom: 16px; }
.spec-close { position: absolute; top: 16px; right: 16px; background: none; border: none; color: #e53e3e; font-size: 16px; cursor: pointer; padding: 4px 6px; border-radius: 4px; line-height: 1; }
...
.image-preview { position: relative; width: 200px; height: 200px; border-radius: 12px; overflow: hidden; border: 1px solid #e0e0e0; }
.image-preview img { width: 100%; height: 100%; object-fit: cover; }
.image-remove { position: absolute; top: 6px; right: 6px; background: rgba(0,0,0,0.45); color: white; border: none; border-radius: 50%; width: 22px; height: 22px; font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
.image-upload-btn { width: 200px; height: 200px; border: 1.5px dashed #c8c8c8; border-radius: 16px; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; background: #f8f8f8; transition: border-color 0.2s, background 0.2s; gap: 8px; }
.image-upload-btn:hover { border-color: #E8572A; background: #fff8f6; }
.image-upload-btn.uploading { cursor: default; border-color: #e0e0e0; }
...
.btn-add-spec { display: block; width: 100%; padding: 18px; margin-top: 16px; background: #fff; border: 1.5px dashed #d0d0d0; border-radius: 14px; color: #888; font-size: 14px; cursor: pointer; text-align: center; transition: border-color 0.2s, color 0.2s, background 0.2s; }
...
.modal-add-btn {
  margin: 0 24px 4px; padding: 12px; border: 1.5px dashed #d1d5db; border-radius: 10px;
  text-align: center; font-size: 14px; color: #6b7280; cursor: pointer;
}
...
.modal-list-item {
  ...
  padding: 14px 16px; border: 1px solid #e5e7eb; border-radius: 10px; margin-bottom: 8px; gap: 8px;
}
