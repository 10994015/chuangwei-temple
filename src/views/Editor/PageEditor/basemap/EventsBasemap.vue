  ...
  <section class="events-section" :class="`device-${device}`">
    <div class="container">
      ...
      <div class="events-carousel">
        ...
        <div class="events-wrapper">
          ...
          >
            <div 
              ...
              :key="event.id"
              class="event-card"
              :style="{ minWidth: `calc(${100 / effectiveVisible}% - ${(effectiveVisible - 1) * 1.5 / effectiveVisible}rem)` }"
            >
        ...
        <button class="carousel-btn next" @click="nextSlide" :disabled="currentIndex >= eventsList.length - effectiveVisible">›</button>
      </div>

      <div class="carousel-indicators">
        <button
          v-for="(_, index) in Math.ceil(eventsList.length / effectiveVisible)"
          :key="index"
          class="indicator-dot"
          :class="{ active: Math.floor(currentIndex / effectiveVisible) === index }"
...
const effectiveVisible = computed(() => {
  if (props.device === 'mobile') return 1
  if (props.device === 'tablet') return 2
  return props.visibleCount
})

const currentIndex = ref(0)
watch(effectiveVisible, () => { currentIndex.value = 0 })

const nextSlide = () => { if (currentIndex.value < props.eventsList.length - effectiveVisible.value) currentIndex.value++ }
const prevSlide = () => { if (currentIndex.value > 0) currentIndex.value-- }
const goToSlide = (index) => { currentIndex.value = index * effectiveVisible.value }
...
.view-all {
  ...
  align-items: center;
  gap: 4px;
  &:hover { color: #6a5a47; transform: translateX(4px); }
}
...
.events-wrapper { flex: 1; overflow: hidden; }
...
.event-card {
  ...
  &:hover {
    ...
    .image-overlay { background: linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.7) 100%); }
  }
}
...
.date-day   { font-size: 28px; font-weight: 700; color: #e74c3c; line-height: 1; margin-bottom: 2px; }
.date-month { font-size: 12px; font-weight: 600; color: #7f8c8d; line-height: 1; }
...
.event-tag {
  ...
  &.hot         { background: linear-gradient(135deg, #e74c3c 0%, #c0392b 100%); }
  &.recommended { background: linear-gradient(135deg, #3498db 0%, #2980b9 100%); }
  &.important   { background: linear-gradient(135deg, #f39c12 0%, #e67e22 100%); }
  &.ceremony    { background: linear-gradient(135deg, #9b59b6 0%, #8e44ad 100%); }
  &.default     { background: linear-gradient(135deg, #95a5a6 0%, #7f8c8d 100%); }
}
...
.event-title {
  ...
  color: #2c3e50; margin: 0 0 1rem 0; line-height: 1.4;
  display: -webkit-box;
  -webkit-line-clamp: 2; -webkit-box-orient: vertical;
  overflow: hidden;
}

.event-details { display: flex; flex-direction: column; gap: 10px; margin-bottom: 1.5rem; }

.event-detail {
  display: flex; align-items: center; gap: 10px;
  font-size: 14px; color: #7f8c8d; line-height: 1.4;
  span { flex: 1; }
}

.detail-icon { width: 18px; height: 18px; color: #95a5a6; flex-shrink: 0; }
...
.view-detail-btn {
  ...
  &:hover  { transform: translateY(-2px); box-shadow: 0 4px 16px rgba(231,76,60,0.4); }
...
.arrow-icon {
  ...
  .view-detail-btn:hover & { transform: translateX(4px); }
}
...
.indicator-dot {
  ...
  &:hover { background: #9ca3af; transform: scale(1.2); }
  &.active { width: 24px; border-radius: 4px; background: #e74c3c; }
}
...
.events-section.device-mobile {
  ...
  .event-details   { gap: 8px; margin-bottom: 1rem; }
  .event-detail    { font-size: 13px; }
