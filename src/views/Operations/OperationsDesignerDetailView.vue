...
onMounted(async () => {
  try {
    data.value = await operationsStore.fetchCustomerDetail(route.params.applicationId)
  } finally {
...
<template>
  <div class="designer-detail-view">
    <nav class="breadcrumb">
      ...
      <span class="sep">›</span>
      <span class="current">創作者詳細資料</span>
    </nav>
    ...
    <h1 class="page-title">創作者詳細資料</h1>
    ...
    <template v-else-if="data">
      <div class="section-card">
        ...
        <div class="info-grid">
          ...
          <div class="info-item">
            ...
            <span class="info-value">{{ data.credential || '-' }}</span>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            ...
            <span class="info-value">{{ genderMap[data.gender] || data.gender || '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">出生日期</span>
            <span class="info-value">{{ fmtDate(data.birthDate) }}</span>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            ...
            <span class="info-value">{{ data.identificationNumber || '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">狀態</span>
            <span class="info-value">
              <span class="badge" :class="{
                'badge-active':   data.status === 'OPEN',
                'badge-inactive': data.status === 'INVALID',
                'badge-lock':     data.status === 'LOCK',
              }">{{ statusMap[data.status] || data.status }}</span>
            </span>
          </div>
          <div class="info-item">
            <span class="info-label">登入方式</span>
            <span class="info-value">{{ data.provider || '-' }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="section-card">
        ...
        <div class="info-grid">
          <div class="info-item">
            ...
            <span class="info-value">{{ fmtDateTime(data.createdAt) }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">最後更新</span>
            <span class="info-value">{{ fmtDateTime(data.updatedAt) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="section-card">
        ...
        <div v-if="!data.tenants || data.tenants.length === 0" class="empty-text">無關聯宮廟</div>
        <table v-else class="data-table">
          ...
          <tbody>
            <tr v-for="t in data.tenants" :key="t.tenantId">
              <td>{{ t.tenantName }}</td>
              <td>{{ t.tenantType }}</td>
            </tr>
          </tbody>
    ...
    <div v-else class="loading">找不到此創作者資料</div>
  </div>
</template>
